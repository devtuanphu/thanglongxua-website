...
export async function generateMetadata({
  ...
  const dataHome = await fetchWithToken(
    `${ENDPOINT.GET_BLOG}?${formattedSearchParams}&filters[slug][$eq]=${slug}`
  );
...
const page = async ({ params }: { params: { slug: string } }) => {
  const { slug } = params;
  const blogDetail = await fetchWithToken(
    `${ENDPOINT.GET_BLOG}?${formattedSearchParams}&filters[slug][$eq]=${slug}`
  );
  const dataBlog = blogDetail?.data[0]?.attributes;
  if (!dataBlog) {
    return (
  ...
  return (
    <div className="container p-4">
      <h1 className="text-2xl font-bold">{dataBlog.title}</h1>
      <p className="text-gray-500">
        {new Date(dataBlog.createdAt).toLocaleDateString()}
      </p>
